...
3. **`Purchasely.closeAllScreens()`** — force-dismiss the paywall, including Flow paywalls with multiple steps.

> **The order `proceed → closeAllScreens` is non-negotiable.** The interceptor must learn the action was handled **before** the paywall tears down. Reversing them produces frozen UIs and double-action bugs.

## SDK version requirements

`closeAllScreens()` is the universally correct dismissal method (it tears down multi-step Flow paywalls correctly). It requires:
...
| React Native | Bridge to native — ensure both iOS pod and Android dependency meet the versions above. |
| Flutter | Same — verify both native sides. |
| Cordova | Same. |

If you cannot upgrade, fall back to `closeDisplayedPresentation()` (older API) but be aware it does not correctly dismiss Flow paywalls.
...
- On React Native / Flutter / Cordova, the JS-side `await synchronize()` resolves once the native bridge confirms; same trade-off as native.
