...
- **Fetch/build a paywall** — native v6: `PLYPresentationBuilder.forPlacementId("id").build().preload()` (iOS) / `PLYPresentation { placementId("id") }.preload()` (Android), keyed on a `placementId` (or `screenId`/`.forScreenId` for a direct Screen). Cross-platform: `Purchasely.fetchPresentation(...)`.
- **Display** — native v6: `presentation.display(from:)` (iOS) / `presentation.display(context)` (Android). Cross-platform: `Purchasely.presentPresentation(...)`, which bridges to native `presentation.display()`.
- **Intercept paywall actions** — native v6: per-action `Purchasely.interceptAction(.purchase) { ... }` returning a `PLYInterceptResult` (`.success`/`.failed`/`.notHandled`). Cross-platform: single `setPaywallActionInterceptor` + `onProcessAction(true/false)`. The v5 native `setPaywallActionsInterceptor` is removed.
...
> **iOS / Android columns are SDK v6.** React Native / Flutter / Cordova columns are all v5.7.x (the v6 Flutter API ships in the final 2.0.0 release).

| Area | iOS (v6) | Android (v6) | React Native | Flutter | Cordova |
|------|-----|---------|--------------|---------|---------|
| Init | `Purchasely.apiKey("K").runningMode(.full).storekitSettings(.storeKit2).logLevel(.debug).start()` (async/throws) or `.start { error in }` | `Purchasely { apiKey("K"); stores(listOf(GoogleStore())); runningMode(PLYRunningMode.Full); onInitialized { error -> } }` or `Purchasely.Builder(ctx).apiKey(...).build().start { error -> }` | `Purchasely.start({ apiKey, storeKit1?, logLevel? })` | `Purchasely.start(apiKey: "K", androidStores: ["Google"], storeKit1: true, logLevel: PLYLogLevel.error, runningMode: PLYRunningMode.full)` → `Future<bool>` | `Purchasely.start(apiKey, stores, userId, logLevel, runningMode, success, error)` |
| Display / Flow | `PLYPresentationBuilder.forPlacementId("id").build().preload()` then `presentation.display(from:)`; use `presentation.controller` (UIKit) / `presentation.swiftUIView` (SwiftUI) only for explicit embedded/nested containers | `PLYPresentation { placementId("id") }.preload()` then `loaded.display(context)`; use `buildView(context) { outcome -> }` (wrap in `AndroidView` for Compose) / `getFragment { outcome -> }` only for explicit embedded/nested containers | `fetchPresentation({ placementId, presentationId?, contentId? })` then `presentPresentation({ presentation, isFullscreen?, loadingBackgroundColor? })` | `Purchasely.fetchPresentation(placementId, {presentationId?, contentId?})` then `Purchasely.presentPresentation(presentation, isFullscreen: false)`; or `Purchasely.presentPresentationForPlacement(placementId, {contentId?, isFullscreen?})` | `fetchPresentationForPlacement(placementId, contentId, success, error)` then `presentPresentation(presentation, isFullscreen, backgroundColor, success, error)` |
| Action interceptor | per-action `Purchasely.interceptAction(.purchase) { info, params in … return PLYInterceptResult }` (async) or `(.purchase) { info, params, completion in completion(.success) }` | per-action `Purchasely.interceptAction<PLYPresentationAction.Purchase> { info, purchase -> PLYInterceptResult.NOT_HANDLED }` (Kotlin) / `interceptAction(PLYPresentationAction.Purchase.class, (info, action, result) -> result.invoke(...))` (Java) | `setPaywallActionInterceptor((result) => { … Purchasely.onProcessAction(true/false) })` | `Purchasely.setPaywallActionInterceptorCallback((PaywallActionInterceptorResult result) { … Purchasely.onProcessAction(true/false) })`; switch on `result.action` (`PLYPaywallAction.purchase`, `.close`, …) | `setPaywallActionInterceptor(cb)` + `Purchasely.onProcessAction(true/false)` |
...
| Synchronize | `synchronize(success:failure:)` | `synchronize()` fire-and-forget | `synchronize(): void` fire-and-forget | `Purchasely.synchronize()` → `Future<void>` | `synchronize()` fire-and-forget |
| Close / dismiss | `closeAllScreens()` native | `closeAllScreens()` native | `closePresentation()` public bridge | `Purchasely.closePresentation()` (and `hidePresentation()` / `showPresentation()`) public bridge | `closePresentation()` public bridge |
...
1. **Interceptor never signals completion**: Native v6 has NO `processAction`/`proceed` — each `Purchasely.interceptAction(...)` handler MUST return a `PLYInterceptResult` (`.success`/`.failed`/`.notHandled`) on every path, or the paywall freezes. Cross-platform (RN/Cordova): every path MUST call `onProcessAction(true/false)`. (Mapping: v5 `processAction(false)` → `.success`; `processAction(true)` → `.notHandled`.)
...
9. **Flow display path**: For Flow paywalls, answer with build/preload (native v6) or `fetchPresentation()` (cross-platform) -> type guard -> `display()` / platform-specific `presentPresentation`. Native v6: `PLYPresentationBuilder.forPlacementId(...).build().preload()` (iOS) / `PLYPresentation { placementId(...) }.preload()` (Android) -> `presentation.display(...)`. Cross-platform bridge syntax: React Native `Purchasely.presentPresentation({ presentation })`; Cordova `Purchasely.presentPresentation(presentation, isFullscreen, backgroundColor, success, error)`; Flutter `Purchasely.presentPresentation(presentation, isFullscreen: ...)`. The bridge calls invoke native `presentation.display()` for Flows. Do not recommend the placement shorthand for Flow display questions.
...
16. **Campaigns are SDK-managed, not app-coded**: A Console **Campaign** with an event trigger (e.g. `APP_STARTED`) is displayed **automatically by the SDK** — the app does NOT build/`fetchPresentation` / `presentPresentation` for it. The only required code is the deeplink display flag: native v6 `allowDeeplink(true)` (default already true; Android auto-intercepts), cross-platform v5 `Purchasely.readyToOpenDeeplink(true)` once the splash/onboarding is ready. Do not conflate Campaigns with Placements: a Campaign can be triggered by an event, by a placement, or both. Trigger-based delivery is automatic; placement-based delivery overrides the placement's default screen when the app builds/fetches that placement. See `references/concepts/campaigns.md`.
...
- `paywall-actions.md` — interceptor (native v6 `interceptAction` + `PLYInterceptResult`; cross-platform `proceed/processAction`) + chaining multiple actions on a single button (purchase + open_screen / open_placement / deeplink)
...
- `observer-mode-post-purchase.md` — dismiss ordering (native v6: `synchronize()` → `closeAllScreens()` → return `PLYInterceptResult.SUCCESS`; cross-platform: `proceed/processAction → dismiss`)
...
- `campaigns.md` — no-code Console automations, deeplink display flag (native v6 `allowDeeplink` / cross-platform `readyToOpenDeeplink`), SDK ≥ 5.1.0
