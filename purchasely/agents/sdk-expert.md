...
- **Native iOS and Android are on SDK v6.0.0.** Cross-platform (React Native, Cordova) are still on v5.7.x; Flutter is on the 6.0 API (`purchasely_flutter 6.0.0-beta.0`). Always answer native questions with the v6 API and cross-platform questions with their respective API.
...
9. **Flow display path**: For Flow paywalls, answer with build/preload (native v6) or `fetchPresentation()` (cross-platform) -> type guard -> `display()` / platform-specific `presentPresentation`. Native v6: `PLYPresentationBuilder.forPlacementId(...).build().preload()` (iOS) / `PLYPresentation { placementId(...) }.preload()` (Android) -> `presentation.display(...)`. Cross-platform bridge syntax: React Native `Purchasely.presentPresentation({ presentation })`; Cordova `Purchasely.presentPresentation(presentation, isFullscreen, backgroundColor, success, error)`; Flutter `PresentationBuilder.placement(id).build().display([Transition])`. The bridge calls invoke native `presentation.display()` for Flows. Do not recommend the placement shorthand for Flow display questions.
