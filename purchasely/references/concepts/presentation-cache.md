...
Fetching a presentation on every display hits the network each time. (Native iOS/Android v6 fetch with `PLYPresentationBuilder` / the `PLYPresentation { }` DSL + `preload`; Flutter v6 builds a request with `PresentationBuilder.placement(id).build()` then `request.preload()`; method-based React Native v5 / Cordova v6 bridges call `Purchasely.fetchPresentation(...)`.) If you display the same placement repeatedly (`onAppear` / `onViewWillAppear` firing multiple times, sheet/back navigation, recomposition, etc.), each fetch:
...
fetchOrCached(placementId):
    ...
    # method-based React Native v5 / Cordova v6 bridges: await Purchasely.fetchPresentation(placementId)
