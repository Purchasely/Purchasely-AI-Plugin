...
| [paywall-actions.md](paywall-actions.md) | `PLYPresentationAction` + per-action `interceptAction` / `PLYInterceptResult` (native iOS/Android + Flutter v6) vs `proceed/processAction` (method-based bridges — React Native v5 / Cordova v6) + chaining multiple actions on a single button (purchase + open_screen / open_placement / deeplink) |
...
| [observer-mode-post-purchase.md](observer-mode-post-purchase.md) | resolve-interceptor → dismiss ordering (v6: no auto-close in Observer), native vs bridge close APIs, chaining follow-up placements |
...
| [campaigns.md](campaigns.md) | No-code Console automations (trigger / placement-based), `allowCampaigns` (native iOS/Android + Flutter v6) / `allowDeeplink` (Cordova v6) / `readyToOpenDeeplink` (React Native v5), SDK ≥ 5.1.0 |
