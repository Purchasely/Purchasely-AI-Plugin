...
## iOS: paywall price shows in USD (wrong currency) in Sandbox & TestFlight

**Symptom:** a paywall displays a `$` price (e.g. `$17.49` / `$68.99`) while the device Region and the store account are set to a Eurozone (or other non-USD) country; the App Store product page **and the native purchase sheet both show the correct local currency** (e.g. `19,99 €` / `79,99 €`).

**Known fix:** none needed — this is an **Apple Sandbox/TestFlight limitation, not a Purchasely bug**, and it does not occur on a production App Store build. In Sandbox/TestFlight the product-metadata request the SDK uses to populate the paywall (`SKProductsRequest` / StoreKit 2 `Product.products(for:)`) frequently returns prices in USD regardless of the signed-in account's storefront, whereas Apple resolves the native purchase confirmation sheet live against the real storefront so it stays correct. The tell-tale sign is exactly this mismatch: correct currency in the Apple purchase popup, `$` on the Purchasely paywall. The returned currency follows the **App Store account storefront**, not the device Region/Language setting — so changing the device Region does not fix it. Confirm on a production/App Store build before treating it as an app, SDK, or Console issue (e.g. a hard-coded `$` price tag). To re-check in TestFlight without waiting for release, tapping **Restore** / re-authenticating with an Apple ID on the target storefront can force StoreKit to return the correct currency. Apple references: [thread/705895](https://developer.apple.com/forums/thread/705895) (Apple App Store Commerce engineer confirms currency follows the signed-in account's storefront, sandbox/TestFlight-specific), [thread/732947](https://developer.apple.com/forums/thread/732947) (SKProduct price differs from the purchase modal), [thread/706641](https://developer.apple.com/forums/thread/706641). Full doc: <https://docs.purchasely.com/docs/testing>.
