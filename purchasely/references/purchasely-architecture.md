...
On app launch, the SDK is initialized with the init builder (native v6: `Purchasely.apiKey(...)....start()` on iOS, the `Purchasely { ... }` DSL on Android; cross-platform: `Purchasely.start(...)`), passing your API key and the user's `userId`, and — when the app expects Purchasely to own the purchase flow — setting the running mode to Full explicitly (native v6 defaults to Observer). It pulls the configuration (paywalls, audiences, active experiments) from the Purchasely Server.
