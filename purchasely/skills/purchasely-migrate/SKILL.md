...
The Flutter plugin migration **adapts the integration to the Purchasely 6.0 native SDKs** (iOS `Purchasely 6.0.0-rc.1`, Android `io.purchasely:core 6.0.0-rc.1`). All public Dart types now carry the **`PLY` prefix** (`PLYPresentationBuilder`, `PLYPresentationOutcome`, `PLYTransition`, …). The one exception is SDK init: `Purchasely.apiKey(...)` (a static method returning a `PurchaselyBuilder`) — this renaming is a **source-breaking change** that landed on 2026-06-24 and applies on top of the v6 API changes. Three areas are breaking: **starting the SDK**, **displaying / preloading / closing a presentation**, and the **action interceptor**. Everything else on the `Purchasely` class is source-compatible.
