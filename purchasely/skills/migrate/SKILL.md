...
4. Update Gradle first:
   ...
   - Move to the Gradle/AGP/Kotlin versions required by the SDK (Gradle 9.3.0+, AGP 9, Kotlin 2.2.x / K2, JDK 11, `minSdk 23`, `compileSdk 36`). With AGP 9, remove `org.jetbrains.kotlin.android` (root `apply false`, every module `plugins { }` block, and the version catalog) — Kotlin support is built into AGP. Leaving it applied fails with `Cannot add extension with name 'kotlin', as there is an extension already registered with that name`. Also remove the `android { kotlinOptions { jvmTarget = "..." } }` block once `kotlin-android` is gone.
