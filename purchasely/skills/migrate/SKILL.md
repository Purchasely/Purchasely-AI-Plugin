---
name: migrate
description: "Use when migrating an existing Purchasely SDK integration between major SDK versions. Supports native Android (Kotlin & Java) and native iOS (Swift & Objective-C) v5.x to v6.0.0 — handles every v5→v6 breaking change so a project can be upgraded in a single prompt."
---

# Purchasely SDK Migration Guide

You are migrating an existing Purchasely SDK integration. You must edit the user's project and verify each migration phase with the platform build/test commands. **Native Android (Kotlin & Java) and native iOS (Swift & Objective-C) v5.x → v6.0.0 are supported.** React Native, Flutter, and Cordova v6 migrations are not ready — stop and explain if asked.
...
4. Update Gradle first:
   ...
   - Move to the Gradle/AGP/Kotlin versions required by the SDK (Gradle 9.3.0+, AGP 9, Kotlin 2.2.x / K2, JDK 11, `minSdk 23`, `compileSdk 35`). With AGP 9, remove `org.jetbrains.kotlin.android` (root `apply false`, every module `plugins { }` block, and the version catalog) — Kotlin support is built into AGP. Leaving it applied fails with `Cannot add extension with name 'kotlin', as there is an extension already registered with that name`. Also remove the `android { kotlinOptions { jvmTarget = "..." } }` block once `kotlin-android` is gone.
