...
- [ ] **SDK version is current** — Compare the pinned version against `../../references/sdk-versions.md` (native iOS and Android on **6.0.0-rc1**; RN/Cordova on 5.7.3; Flutter on 5.7.3). FAIL if older than minimum, WARNING if not at latest. FAIL if a floating version (`5.+`, `6.+`, `^5.0.0`, `~> 6.0`, `from:`/Up-to-Next-Major, etc.) is used instead of an exact pin. Because `6.0.0-rc1` is a pre-release, iOS must pin it exactly too — CocoaPods `pod 'Purchasely', '6.0.0-rc1'`, SPM `exact: "6.0.0-rc1"`, Carthage `== 6.0.0-rc1`.
