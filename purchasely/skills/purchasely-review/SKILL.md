...
- `../../references/concepts/campaigns.md` — deeplink display flag (`readyToOpenDeeplink` on v5 RN; renamed `allowDeeplink`, default `true`, on v6 — native iOS/Android, Flutter, and Cordova) + SDK ≥ 5.1.0
...
- Confirm the SDK generation used for each finding: native iOS / native Android / Flutter / Cordova use v6 (`6.0.0-rc.1`); React Native uses v5 (`5.7.3`).
- Confirm version findings compare against `../../references/sdk-versions.md`.
- Confirm Full-vs-Observer findings account for the v6 default running mode change (which now also applies to Cordova).
- Confirm removed/deprecated API findings are platform-specific and do not flag valid React Native v5 / Cordova v6 APIs (Cordova v6 keeps the method-based JS surface).
...
- Method-based cross-platform (RN v5 / Cordova v6): `Purchasely.start` / `Purchasely.start(` (positional args, never an object) / `withAPIKey`
...
- v5 / removed (flag if present on native or Flutter v6): `fetchPresentation` / `presentationView` / `presentationController` / `presentPresentation` / `presentPresentationForPlacement` / `presentPresentationWithIdentifier`
- Method-based cross-platform (RN v5 / Cordova v6): `fetchPresentation` / `presentPresentation` (Cordova v6 keeps these)
...
- Method-based cross-platform (RN v5 / Cordova v6): `setPaywallActionInterceptor` / `onProcessAction` (Cordova v6 keeps these)
...
**Deeplink patterns:**
- v6 (native, Flutter, Cordova): `handleDeeplink` / `allowDeeplink`
- v5 (RN) / removed-or-renamed in v6 (flag if used as the primary name on native, Flutter v6, or Cordova v6): `isDeeplinkHandled` / `readyToOpenDeeplink`
- `setDefaultPresentationResultHandler` (v5 — renamed `setDefaultPresentationDismissHandler` on Cordova v6) / `setDefaultPresentationDismissHandler`
...
- [ ] **Explicit running mode when purchases are validated by Purchasely** (v6 — native iOS/Android, Flutter, and Cordova) — ⚠️ v6 changed the default running mode from Full to **Observer**, *silently*. If the app expects Purchasely to process/validate purchases but the init does NOT set `.runningMode(.full)` (iOS) / `runningMode(PLYRunningMode.Full)` (Android) / `.runningMode(RunningMode.full)` (Flutter `PurchaselyBuilder`) / `Purchasely.RunningMode.full` as the running-mode positional argument of `Purchasely.start(...)` (Cordova), purchases are never validated. **FAIL** if Full behavior is relied on but the mode is left at the v6 default. SKIP for genuine Observer integrations and for React Native (still on v5, where the default is Full).
- [ ] **start() completion handled** — The completion/callback of init (`.start { error -> }` / `try await ….start()` on native v6; `await PurchaselyBuilder.…start()` on Flutter v6; `Purchasely.start(...)` success/error callbacks on method-based cross-platform RN v5 / Cordova v6) must be awaited or handled before calling other SDK methods like the presentation builder / `fetchPresentation`. FAIL if SDK methods are called in a fire-and-forget pattern after start.
- [ ] **Stores configured correctly** — Android must specify at least one store (`Google`, `Huawei`, `Amazon`); on Flutter v6 via `.stores([PLYStore.google])` on the `PurchaselyBuilder`. iOS does not need store config. Method-based cross-platform SDKs (RN v5 / Cordova v6) must pass the correct store for the target platform in the `Purchasely.start(...)` stores argument. WARNING if stores are missing on Android. Note (v6 — native, Flutter, and Cordova): a **storeless** start is valid for screens/analytics/campaigns; only flag missing stores when the app runs in Full mode and expects purchases (purchase APIs return `PLYError.NoStoreConfigured`).

### 3.2 Paywall Display

- [ ] **Uses the v6 builder/preload API** (native iOS/Android and Flutter) — Must build + preload + display: iOS `PLYPresentationBuilder.forPlacementId(_).build().preload()` → `display(from:)`; Android `PLYPresentation { placementId(...) }.preload()` → `display(context)`; Flutter `PresentationBuilder.placement(id).build()` → `PresentationRequest` then `.preload()` and/or `.display([Transition])` (resolves at dismiss with a `PresentationOutcome`). The v5 `fetchPresentation(...)` / `presentPresentation(...)` / `presentPresentationForPlacement(...)` / `presentPresentationWithIdentifier(...)` / `presentationView(...)` / VC-returning methods are **removed** in v6 — **FAIL** if any appear in native or Flutter v6 code. For embedded use: iOS `controller` / `swiftUIView`, Android `buildView(...)` wrapped in `AndroidView` for Compose (there is no `presentation-compose` artifact or native `PLYPresentationView` composable), Flutter the `PLYPresentationView(request: ...)` widget. For Console-direct Screens, the builder accepts `screenId(...)` / `.forScreenId(_)` / Flutter `.screen(id)`.
- [ ] **Prefers `fetchPresentation` + `presentPresentation` over `presentPresentationForPlacement`** (method-based bridges — React Native v5 / Cordova v6) — The pre-fetch path is what the [official docs recommend](https://docs.purchasely.com/docs/general-in-app-experiences-display#how-to-display-an-in-app-experience-associated-to-a-placement); it's the only one that correctly displays **Flows** (the shorthand routes Flows through the legacy single-paywall path and the user gets a modal with no close affordance). `presentPresentationForPlacement` remains acceptable for placements guaranteed to host only simple non-Flow paywalls — WARNING otherwise, and FAIL if the same code path also relies on Flow steps. Cordova v6 keeps these method-based APIs. SKIP on native iOS/Android and Flutter v6 (use the builder check above — `fetchPresentation`/`presentPresentation*` are removed there).
...
- [ ] **Interceptor is registered** — v6 uses **per-action** registration: `Purchasely.interceptAction(.login) { … }` (iOS) / `Purchasely.interceptAction<PLYPresentationAction.Login> { … }` (Android) / `Purchasely.interceptAction(PresentationActionKind.purchase, (info, payload) async { … })` (Flutter) — one call per action kind you handle, typically right after init. The v5 single `setPaywallActionsInterceptor` (native) / `setPaywallActionInterceptorCallback` + `onProcessAction` (Flutter v5) is **removed** on native and Flutter v6 — FAIL if it appears in native or Flutter v6 code. Method-based cross-platform RN v5 / Cordova v6 use the single `setPaywallActionInterceptor` + `onProcessAction` (Cordova v6 keeps this). WARNING if no interceptor is registered at all and the app needs to handle login/navigate/observer purchases.
...
- [ ] **CLOSE action handled** — The close action must dismiss the paywall. v6 (native + Flutter, kind `close` / `PresentationActionKind.close`): return `.notHandled` to let the SDK close, or `.success` if the app closes it itself (Flutter: via `presentation.close()`). Method-based cross-platform (RN v5 / Cordova v6): `onProcessAction(true)`. FAIL if missing (users cannot close the paywall).
- [ ] **Every cross-platform interceptor path calls `onProcessAction()`** (method-based bridges — RN v5 / Cordova v6) — every branch (early return, error catch, switch default) MUST call `onProcessAction()` or the paywall freezes. This is the #1 most common cross-platform Purchasely bug. FAIL if any code path can skip it. SKIP on native v6 and Flutter v6 (replaced by the `PLYInterceptResult` / `InterceptResult` return-value check above — there is no `onProcessAction`/`processAction`/`proceed` callback). On Flutter v6 the equivalent freeze risk is a handler path that never returns an `InterceptResult` — covered by the return-value check above.
- [ ] **No double-completion** — On method-based cross-platform bridges (RN v5 / Cordova v6), calling `onProcessAction()` twice is undefined; on v6 (native + Flutter), returning then mutating state after the handler resolves is a logic error. WARNING if there's a risk of double signalling.
...
- [ ] **handleDeeplink() called** — `Purchasely.handleDeeplink(...)` must be called when the app receives a URL. On v6 (native, Flutter, and Cordova) this is the rename of v5 `isDeeplinkHandled(...)` — WARNING if the deprecated `isDeeplinkHandled` is still used (Cordova v6 renamed it, not aliased — FAIL if `isDeeplinkHandled` appears in Cordova v6 code). On **iOS** the SDK does NOT auto-intercept, so `handleDeeplink(url)` must be wired from AppDelegate/SceneDelegate. On **Android v6** deeplinks are auto-intercepted (zero code) by reading the foreground activity intent; if the activity is `singleTask`/`singleTop`, verify `setIntent(intent)` is called in `onNewIntent` (otherwise the URI is hidden) or that a manual `handleDeeplink(uri, activity)` call exists. On **Flutter v6** the app should forward incoming links from its deeplink handling (e.g. `uni_links` / `app_links` / router) to `Purchasely.handleDeeplink(uri)` for iOS; Android still benefits from the native auto-interception. On **Cordova v6** forward incoming URLs to `Purchasely.handleDeeplink(url, success, error)`. SKIP if the app doesn't support deeplinks.
- [ ] **allowDeeplink enabled** — v6 (native, Flutter, and Cordova) renamed `readyToOpenDeeplink` → `allowDeeplink` (set via the init builder modifier `.allowDeeplink(true)` / the runtime flag `Purchasely.allowDeeplink(bool)`). It defaults to **true** on v6, so usually no action is needed; WARNING only if the app explicitly sets `allowDeeplink(false)` while also relying on campaign/deeplink display. On React Native (v5) the old `readyToOpenDeeplink(true)` is still required after the UI is initialized. WARNING if the deprecated `readyToOpenDeeplink` name is used as the primary call in v6 native, Flutter v6, or Cordova v6 code (on Cordova v6 it was renamed, not aliased — FAIL there).
- [ ] **setDefaultPresentationDismissHandler configured** — A default dismiss handler should be set so deeplink-triggered paywalls can report their outcome. On Cordova v6 this is the rename of v5 `setDefaultPresentationResultHandler` (renamed, not aliased — FAIL if `setDefaultPresentationResultHandler` appears in Cordova v6 code). WARNING if missing.
...
- [ ] **Manage Subscription entry point** — both stores require an in-app link to native subscription management. WARNING if missing from Settings / Account. See `../../references/concepts/subscription-management.md`. Note (v6 — native, Flutter, and Cordova): the built-in subscriptions screen was **removed**. `Purchasely.presentSubscriptions()` is **removed entirely** from Flutter v6 (it is NOT a no-op — the method no longer exists) — FAIL if it still appears in Flutter v6 code. On **Cordova v6** `Purchasely.presentSubscriptions()` is a **no-op** (it still exists on the JS surface but logs a warning and does nothing — it is NOT removed) — WARNING if relied on for the Manage Subscription UI. On both, build your own screen from `userSubscriptions()` / `userSubscriptionsHistory()`. `Purchasely.displaySubscriptionCancellationInstruction()` is kept for source-compat but is a **no-op** on native.
...
- [ ] **Wrapper owns init and interceptor** — init (`start()`) and the action interceptor registration (`Purchasely.interceptAction(...)` on v6 — native and Flutter; `setPaywallActionInterceptor` on method-based RN v5 / Cordova v6) should be in the wrapper, not scattered. WARNING if init logic is outside.
...
- [ ] **SDK version is current** — Compare the pinned version against `../../references/sdk-versions.md` (native iOS and Android, **Flutter**, **and Cordova** on **6.0.0-rc.1**; React Native on 5.7.3). FAIL if older than minimum, WARNING if not at latest. FAIL if a floating version (`5.+`, `6.+`, `^5.0.0`, `~> 6.0`, `from:`/Up-to-Next-Major, `^6.0.0-rc.1`, etc.) is used instead of an exact pin. Because `6.0.0-rc.1` is a pre-release, it must be pinned exactly on every layer — iOS CocoaPods `pod 'Purchasely', '6.0.0-rc.1'`, SPM `exact: "6.0.0-rc.1"`, Carthage `== 6.0.0-rc.1`; Android `io.purchasely:core:6.0.0-rc.1`; **Flutter** `purchasely_flutter: 6.0.0-rc.1` (and `purchasely_google` / `purchasely_android_player` at the same `6.0.0-rc.1`, never `^`/`>=`); **Cordova** `@purchasely/cordova-plugin-purchasely: 6.0.0-rc.1` (and `@purchasely/cordova-plugin-purchasely-google` at the same `6.0.0-rc.1`), which pulls native iOS `Purchasely 6.0.0-rc.1` and Android `io.purchasely:core 6.0.0-rc.1`.
- [ ] **Plugin packages aligned** (cross-platform only) — All `react-native-purchasely*` (5.x.y on React Native) packages MUST be the same version; on **Flutter** all `purchasely_*` packages (`purchasely_flutter`, `purchasely_google`, `purchasely_android_player`) MUST be the same `6.0.0-rc.1`; on **Cordova** all `@purchasely/cordova-plugin-*` packages (`@purchasely/cordova-plugin-purchasely`, `@purchasely/cordova-plugin-purchasely-google`) MUST be the same `6.0.0-rc.1`. FAIL if mismatched.
...
- [ ] **No removed / deprecated APIs** — Native v6 **removed**: `setPaywallActionsInterceptor`, `fetchPresentation` (native), `presentationView(for:)`/`presentationViewControllerFor`/`presentationController`, `subscriptionsFragment()` and all `PLYSubscriptions*`/`PLYSubscriptionCancellation*` UI, `purchaseHistory()` (→ `userSubscriptionsHistory()`), `isPastSubscriber()`, the `intro*`/`introductory*` plan methods and `PLYPlanTags.INTRO_PRICE`/`TRIAL_PRICE` (→ `offer*` / `PLYPlanTags.OFFER_PRICE`), `PLYPresentationInfo` (→ `PLYInterceptorInfo`), and — **Android only** — `PLYPresentationActionParameters` (Android replaced it with typed action subclasses; **iOS retains** `PLYPresentationActionParameters` as the interceptor `params`, so do not flag it on iOS). **FAIL** for each occurrence in native code (it won't compile against v6). Native v6 **deprecated** (removal v7): `readyToOpenDeeplink` (→ `allowDeeplink`), `isDeeplinkHandled` (→ `handleDeeplink`), pre-`start` class funcs like `setEnvironment`/`setThemeMode` (→ builder modifiers) — WARNING for each.
  ...
  - **Cordova v6 renamed** (FAIL — renamed, not aliased, so the old name no longer resolves): `Purchasely.RunningMode.paywallObserver` (→ `Purchasely.RunningMode.observer`), `readyToOpenDeeplink` (→ `allowDeeplink`), `isDeeplinkHandled` (→ `handleDeeplink`), `setDefaultPresentationResultHandler` (→ `setDefaultPresentationDismissHandler`), `startWithAPIKey` (→ `start`). The Cordova JS surface otherwise keeps its v5 method names and signatures: `Purchasely.start(...)` (positional, **never** an object), `fetchPresentation` / `presentPresentation[ForPlacement]`, `setPaywallActionInterceptor` + `onProcessAction`, `closePresentation()`, every `setUserAttributeWith*`. **Cordova v6 no-op** (WARNING — still on the JS surface, logs a warning, does nothing): `Purchasely.presentSubscriptions()`.
- [ ] **Error handling around presentation build** — The presentation can fail (network error, invalid placement). The error/failure case must be handled gracefully: native v6 `.preload { loaded, error -> }` / `.preload()` throwing; **Flutter v6** the `display([Transition])` future's `PresentationOutcome.error` (and `try/catch` around `.preload()` / `.display()`); method-based RN v5 / Cordova v6 `fetchPresentation` error callback. FAIL if errors are silently ignored. Map known `PLYError` cases (see `../../references/troubleshooting/error-codes.md`) when surfacing failure to the user.
...
- [ ] **Deeplink display enabled** — trigger-based campaigns are delivered through deeplinks. v6 (native, Flutter, and Cordova): `allowDeeplink` defaults to **true**, so usually no action is needed; FAIL only if the app sets `allowDeeplink(false)`, and on Android verify the auto-interception isn't broken (e.g. `singleTask` activity missing `setIntent(intent)`). On Flutter v6 and Cordova v6, also ensure incoming deeplinks reach `Purchasely.handleDeeplink(url)` on iOS (Android auto-intercepts). React Native (v5): `Purchasely.readyToOpenDeeplink(true)` must be called once after the splash/launch routine completes — FAIL if missing, WARNING if called inside the `start()` callback (the campaign paywall lands on top of the splash).
...
1. **[FAIL] Interceptor handler does not return a result on every path** — `PaywallInterceptor.kt:45`
   The catch block on line 45 returns without producing a `PLYInterceptResult`. On v6 (native + Flutter) every branch of the handler must return `.success` / `.failed` / `.notHandled` (Flutter: `InterceptResult.*`). (On method-based bridges — RN v5 / Cordova v6 — the equivalent bug is a path that never calls `onProcessAction()`, which freezes the paywall.)
