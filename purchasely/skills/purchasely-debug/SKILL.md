...
- **Cross-platform (RN / Cordova):** `onProcessAction()` was not called in every code path.
...
3. **Observer mode**: the app handles purchases itself. After a successful purchase, `Purchasely.synchronize()` must be called so the SDK validates the receipt. Native v6 Observer mode also does NOT auto-close the paywall — dismiss with `closeAllScreens()` and return `PLYInterceptResult.SUCCESS`.
...
| Paywall doesn't dismiss after Observer-mode purchase | v6 Observer mode no longer auto-closes, or the dismiss/signal order is wrong | Native iOS/Android v6: inside the `.purchase` handler, `synchronize()` → `closeAllScreens()` → `return PLYInterceptResult.SUCCESS` (the explicit dismiss is required in v6). iOS `closeAllScreens()` is `@MainActor`-isolated — wrap in `Task { @MainActor in … }` from non-isolated contexts. Cross-platform (RN / Cordova): `onProcessAction(false)` → `closePresentation()`, in that order |
