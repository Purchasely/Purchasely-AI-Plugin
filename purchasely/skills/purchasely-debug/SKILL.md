...
| Cordova (v6): choosing a dismissal method | Cordova exports `Purchasely.closeAllScreens()` as a JavaScript API, and a built presentation request also exposes `request.close()`; both invoke the native `closeAllScreens` action and dismiss all displayed screens | Use `request.close()` as a convenience when you hold the presentation request, or call `Purchasely.closeAllScreens()` directly. `Purchasely.closePresentation()` remains a deprecated alias for the top-level method |
