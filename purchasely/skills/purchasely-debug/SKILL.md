...
1. **Check handler method** -- native v6 / Flutter v6: `handleDeeplink(...)` (Flutter v6 keeps `isDeeplinkHandled` as a deprecated alias). ⚠️ **React Native v6 renamed the method to `Purchasely.handleDeeplink(uri)`** (the v5 `isDeeplinkHandled` is removed with no alias, matching native iOS/Android and Flutter) — if RN code still calls `isDeeplinkHandled`, that's the bug; rewrite it to `handleDeeplink`. Cordova (v5): `isDeeplinkHandled`.
