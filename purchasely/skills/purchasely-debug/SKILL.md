...
- `../../references/concepts/campaigns.md` — trigger-based campaigns silently don't fire (missing `readyToOpenDeeplink` on v5 React Native; on v6 — native iOS/Android, Flutter, and Cordova — it is `allowDeeplink`, default `true`, displayed immediately, and Android auto-intercepts)
...
- Confirm the SDK generation: native iOS / native Android / Flutter / Cordova use v6 (`6.0.0-rc.1`); React Native uses v5 (`5.7.3`).
...
1. **Check SDK initialization** -- native v6: search for the init builder (`Purchasely.apiKey(` on iOS, `Purchasely {` / `Purchasely.Builder(` on Android) and `.start(`; Flutter v6: search for `PurchaselyBuilder.apiKey(` and `.start()` (the v5 `Purchasely.start({...})` is gone); cross-platform (method-based: RN v5 / Cordova v6): `Purchasely.start(`. Verify the start callback/completion succeeds without errors (native v6 callback is `start { error -> }` / `start { error in }`, a single nullable `PLYError`; Flutter v6 `.start()` returns a `Future<bool>`).
2. **Check placement / builder** -- native v6: find the `PLYPresentation { placementId(...) }` (Android) / `PLYPresentationBuilder.forPlacementId(...)` (iOS) call and confirm `.preload()` (or `.preload { … }`) is actually invoked — a built-but-never-preloaded/displayed presentation shows nothing. Flutter v6: find the `PresentationBuilder.placement(...)` / `.screen(...)` call and confirm `.build()` is followed by `.display(...)` (or `.preload()` then `.display(...)`) — a request that is built but never displayed shows nothing (the v5 `fetchPresentation(`/`presentPresentationForPlacement(` are gone). Cross-platform (method-based: RN v5 / Cordova v6): find the `fetchPresentation(` call. Verify the placement ID string matches one active in the Console (also accept a `screenId(...)` / `.forScreenId(...)` / Flutter `.screen(...)` for a direct Screen).
...
- **Cross-platform (method-based: RN v5 / Cordova v6):** `onProcessAction()` was not called in every code path.

1. **Find the interceptor** -- native v6 / Flutter v6: search for `Purchasely.interceptAction`. Cross-platform (method-based: RN v5 / Cordova v6): `setPaywallActionInterceptor` / `onProcessAction`. Older native code may still reference the removed `setPaywallActionsInterceptor` — that won't compile against v6.
2. **Audit every code path** -- native v6: every branch (success, failure, cancellation, timeout) MUST return a `PLYInterceptResult`. Flutter v6: every branch MUST return an `InterceptResult`. Cross-platform (method-based: RN v5 / Cordova v6): every branch MUST call `onProcessAction(true/false)`. A missing return / missing call freezes the paywall.
...
4. **Check try/catch blocks** -- native v6: a caught exception must still `return .failed` (or `.notHandled`). Flutter v6: a caught exception must still `return InterceptResult.failed` (or `.notHandled`). Cross-platform (method-based: RN v5 / Cordova v6): it must still call `onProcessAction(false)`.
5. **Fix**: ensure every exit path produces a result. Native v6: wrap in `do/catch` (Swift) / `try/finally` (Kotlin) and return `.failed` on error. Flutter v6: wrap in `try/catch` and `return InterceptResult.failed` on error. Cross-platform (method-based: RN v5 / Cordova v6): `try/finally` calling `onProcessAction(false)`.
...
1. **Check handler method** -- native v6 / Flutter v6 / Cordova v6: `handleDeeplink(...)` (native + Flutter v6 keep `isDeeplinkHandled` as a deprecated alias; **Cordova v6 renamed it — `isDeeplinkHandled` no longer resolves, use `handleDeeplink`**); React Native v5: `handleDeeplink` (current) vs `isDeeplinkHandled` (deprecated). If the deprecated name is used, recommend switching.
2. **Check the deeplink display flag** -- native v6, Flutter v6, and Cordova v6 renamed `readyToOpenDeeplink` → `allowDeeplink`, default **true** (so usually nothing to set; v6 displays deeplinks/campaigns immediately by default). Native + Flutter v6 keep `readyToOpenDeeplink` as a deprecated alias; **Cordova v6 renamed it (no alias) — call `Purchasely.allowDeeplink(bool)`, not `readyToOpenDeeplink`**. On React Native v5 `readyToOpenDeeplink(true)` must be set after the UI is ready. If an app explicitly set `allowDeeplink(false)`, deeplinks won't display.
...
4. **Check default presentation dismiss handler** -- a default handler must be configured, or the SDK has nowhere to send deeplink paywall results. On Cordova v6 this is `setDefaultPresentationDismissHandler` (renamed from v5 `setDefaultPresentationResultHandler`, no alias — the old name no longer resolves).
...
4. **If the cause is unclear**, suggest adding temporary debug logging at key points:
   ...
   - Where the presentation finishes loading (native v6: the builder `onPresented` / `.preload` result — log type + error; Flutter v6: the builder `.onPresented` / `request.preload()` result — log `presentation.type` + `outcome.error`; cross-platform method-based RN v5 / Cordova v6: the `fetchPresentation` callback)
   - In every branch of the action interceptor
   - At each interceptor exit (native v6: log the returned `PLYInterceptResult`; Flutter v6: log the returned `InterceptResult`; cross-platform method-based RN v5 / Cordova v6: before/after `onProcessAction`)
...
| method-based RN v5 / Cordova v6: same stuck-paywall / repeated-fetch issue as iOS above | Same SDK quirk — the method-based bridge calls native fetch every time and has no shared cache | Apply the universal cache pattern from `../../references/concepts/presentation-cache.md` (skeleton implementations included for RN, Cordova) |
...
| method-based RN v5 / Cordova v6: `closeAllScreens()` not exposed on JS side | Expected public bridge API mismatch | Use `closePresentation()` on the public JS side. `closeAllScreens()` is the native iOS/Android method unless the app has added a custom bridge |
...
| method-based RN v5 / Cordova v6: native crash on init or missing API | Plugin packages out of alignment (e.g. RN `react-native-purchasely 5.7.3` + `@purchasely/react-native-purchasely-google 5.6.0`; or Cordova `@purchasely/cordova-plugin-purchasely 6.0.0-rc.1` + `@purchasely/cordova-plugin-purchasely-google 5.7.3`) | Pin all plugin packages to the same version — React Native on `5.7.3`, Cordova on `6.0.0-rc.1`. See `../../references/sdk-versions.md` |
