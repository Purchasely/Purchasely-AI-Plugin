...
## [2.0.0-rc.4] — 2026-06-23

React Native joins the **v6 line**. React Native guidance moves off v5 to match native iOS, Android & Flutter — the published `react-native-purchasely` now targets **Purchasely v6.0.0-rc.2** with the builder API (native iOS / Android / Flutter guidance stays on `6.0.0-rc.1`). **Cordova stays on v5 (`5.7.3`).**

### Changed

- **All skills updated for React Native v6.** `purchasely-integrate`, `purchasely-review`, `purchasely-debug`, `purchasely-migrate`, and the `purchasely-sdk-expert` skill now document the **React Native v6 API**: `Purchasely.builder('key')` fluent init with string options (default running mode `'observer'`), `Purchasely.presentation` (`PLYPresentationBuilder`, factories `.placement()` / `.screen()` / `.defaultSource()`) / `PLYPresentationRequest` (`build().preload()` → `PLYLoadedPresentation`, `.display()`, replacing `fetchPresentation` / `presentPresentation[ForPlacement]`), per-action `Purchasely.interceptAction(kind, handler)` returning `'success' | 'failed' | 'notHandled'` (replacing `setPaywallActionInterceptorCallback` + `onProcessAction`), `request.close()` to dismiss (Android closes all displayed presentations, iOS closes the targeted one), `Purchasely.setDefaultPresentationDismissHandler`, and awaitable `synchronize()`. Deeplinks: `isDeeplinkHandled` / `readyToOpenDeeplink` are **removed** — use `Purchasely.handleDeeplink(uri)` and `.allowDeeplink(true)`. `PLYPaywallAction` and `RunningMode.TRANSACTION_ONLY` / `PAYWALL_OBSERVER` are removed. React Native is carved out of the former "v5 cross-platform" grouping; Cordova keeps its v5 guidance.
- **`references/react-native/integration.md` rewritten as a shippable v6 doc**, plus new **`references/react-native/migration-v6.md`** and **`references/react-native/v5-api-reference.md`**; pin `react-native-purchasely` and the `@purchasely/react-native-purchasely-*` store packages to `6.0.0-rc.2`. React Native now requires Android `minSdk 23` (builds against `compileSdk 35`, RN 0.86 / Node 22).
- **`references/sdk-versions.md`** — the React Native row, npm pins, and transitive-version notes move to `6.0.0-rc.2` (which pulls the `6.0.0-rc.2` native SDKs). native iOS / Android / Flutter rows kept on `6.0.0-rc.1`; Cordova row kept on `5.7.3`.
- **`purchasely-migrate`** now migrates React Native v5.x → `6.0.0-rc.2` (alongside native iOS, Android & Flutter), with a dedicated React Native workflow and `yarn typecheck` / `yarn lint` / `yarn test` + example-app build verification.
- Concept and troubleshooting references (running modes, observer-mode post-purchase, paywall actions, presentation types/cache, promotional offers, campaigns, architecture, common issues, debug mode) updated so React Native examples use the v6 API.
- **`scripts/guard-known-bad-snippets.mjs`** — React Native is exempted from the v5 `allowDeeplink` guard (it now documents the v6 builder API); Cordova stays guarded on v5.

### Removed

- React Native docs no longer present `Purchasely.presentSubscriptions()` — it is **removed** in React Native v6 (the native subscription-list UI was dropped on both platforms; build your own from `userSubscriptions()` / `userSubscriptionsHistory()`).
