...
- `purchasely/skills/purchasely-debug/SKILL.md`: clarified that Cordova's `request.close()` and `Purchasely.closeAllScreens()` both dismiss all displayed screens, and documented `request.close()` as a convenience on the held request.
...
- Android v6 additions documented: zero-code automatic deeplink handling, `themeMode()` at init, structured `PLYTransitionDimension`, bundled lint checks.
- Release process section in `CLAUDE.md`.
...
- React Native `allowDeeplink` default corrected (native default `true`, no RN exception).
- Removed-API names corrected per platform: iOS `showController` / `PLYUIControllerType` (`presentSubscriptions()` never existed on iOS); Android `subscriptionsFragment()`; `displaySubscriptionCancellationInstruction()` is removed, not a no-op.
- Android default dismiss handler name corrected (`setDefaultPresentationDismissHandler`).
- Native iOS requirement corrected to 13.4+ (inherited from 5.x).
- Flutter concept pages updated to `PLY`-prefixed Dart types.
...
- Flutter v6 references now use the **`PLY`-prefixed Dart types** (2026-06-24 rename): `PLYPresentationBuilder`, `PLYPresentationRequest`, `PLYPresentation`, `PLYPresentationOutcome`, `PLYTransition` / `PLYTransitionDimension`, `PLYRunningMode`, `PLYPresentationActionKind`, `PLYInterceptResult`, `PLYPurchaseResult`, typed `PLY*Payload`, etc. SDK init is still accessed via `Purchasely.apiKey(...)` (returns `PurchaselyBuilder`). `PLYRunningMode` has only `observer` / `full`; interceptor cleanup is `removeActionInterceptor` / `removeAllActionInterceptors`; the default dismiss handler is `Purchasely.setDefaultPresentationDismissHandler`.
- **Native SDK pin → `6.0.0-rc.2`.** `references/sdk-versions.md` and the Flutter references now reflect the published `6.0.0-rc.2` native SDKs (iOS `Purchasely`, Android `io.purchasely:core`); the Flutter Dart packages stay `6.0.0-rc.1` and pull rc.2 transitively.
